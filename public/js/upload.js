...
function handleFileSelect(e) {
    ...
    var maxSize = $(e.target).data('max-size');

    filesArr.forEach(function(f) {
        var fileSize = Math.round(f.size/1024); // in kb
        if(fileSize >= maxSize){
            alert('Maximum image size exceed.');
            $("#files").val("");
            return;
        }
        ...
        reader.onload = function(e) {
            var html = "<div><img src=\"" + e.target.result + "\" data-file='" + f.name + "' class='selFile' title='Click to remove'></div>";
