$(document).ready(function() {
    ...
    $('#type').on('change',function(){
        if( $(this).val()==="standlone"){
            ...
            var html = '<div class="form-group col-xs-12 col-sm-7">' +
                            '<label for="link" >Link</label>' +
                            '<input class="form-control" type="url" id="link" name="link" value=""/>' +
                        '</div>' +
                        '<div class="col-xs-12">' +
                            '<label for="files" > Desktop</label>' +
                            '<input class="form-group" data-device="desktop" type="file" id="files" name="files[]" multiple required=""> ' +
                            '<div id="selectedFilesD"> </div> ' +
                        '</div>' +
                        '<div class="col-xs-12">' +
                            '<label for="mobile">Mobile </label>' +
                            '<input class="form-group" data-device="mobile" type="file" id="mobile" name="mobile[]" multiple> ' +
    ...
    filesArr.forEach(function(f) {
        ...
        reader.onload = function(e) {
            ...
            var html = "<div class='col-xs-6 col-md-3'>" +
                            "<img class='thumbnail' src=\"" + e.target.result + "\" data-file='" + f.name + "' class='selFile' title='Click to remove'>" +
                            "<p class='pname'>" +f.name + "</p>" +
                        "</div>";
